...
/* -- [ VERFICAMOS SE EXISTE O DIRETÓRIO DO MÓDULO ] -- */
...
/* -- [ MOSTRAMOS OS MÓDULOS EXISTENTES ] -- */	
...
/* -- [ TEXTOS LEITURA ] -- */
				...
				$o = new printConteudo(" <b id='destaque5'>$_TEXTOS_PARA_LEITURA</b>");
				$MJO->NTD('align="left" class='.$COR1.'');	
				$FORM->comboSIM_NAO("TextoLeituraTrocado",$TextoLeitura,$_SIM,$_NAO);								
				$MJO->NTD('align="left" class='.$COR1.'');
				$F->ajuda2 ("","",$_DICA_CAMPO_MODULO,$CFG->iconeAjuda,$_MODULO,$_MODULO);									
				$MJO->NCTR();
  			
/* -- [ STATUS DE TEXTOS LEITURA ] -- */
				
				$MJO->NTR('align="LEFT" COLSPAN="3" vAlign="top" class='.$COR1.'');
						$o = new printConteudo("<b id='destaque4'>$_STATUS_ATUAL</b>");
					if($TextoLeitura == "UM"){
						$o = new printConteudo("<b id='destaque3'>$_SIM</b>");
					}else{						
						$o = new printConteudo("<b id='destaque10'>$_NAO</b>");
					}
				$MJO->NCTR();
...
/* -- [ AUTO-CADASTRO ] -- */
				...
				$MJO->NTD('align="left" class='.$COR7.'');
					$FORM->comboSIM_NAO("autoCadastroTrocado",$autoCadastro,$_PERMITE_AUTOCADASTRO,$_NAO_PERMITE_AUTOCADASTRO);
				$MJO->NTD('align="left" class='.$COR7.'');
...
/* -- [ LINK RETORNO / NOME EVENTO ] -- */
				if($VersaoDemo == "1"){
				$F->inputDica('','','','TYPE="hidden" name="linkRedirecionaRetornoTrocado" value="'.$linkRedirecionaRetorno.'"');
				}

				$F->inputTD('colspan="3"',$classeLinha1,'<b id="destaque5">'.$_LINK_RETORNO.'</b>','<b id="destaque5">'.$_NOME_EVENTO.'</b>',$classeLinha1,'TYPE="TEXT" CLASS='.$classeInput.' '.$desabilitar.' TITLE="'.$_TITULO_DESABILITADO.'" name="linkRedirecionaRetornoTrocado" value="'.$linkRedirecionaRetorno.'" size="20"','TYPE="TEXT" name="nomeEventoTrocado" value="'.$nomeEvento.'" size="20"',$_DICA_CAMPO_LINK_RETORNO,$_DICA_CAMPO_NOME_EVENTO,$CFG->iconeAjuda,$_LINK_RETORNO,$_NOME_EVENTO);
...
if(isset($_POST["alterar"])){
	...
	if ($arquivo) {
		...
		42 =>'$TextoLeitura = "'.$TextoLeituraTrocado.'";',
		43 =>'$repositorioArquivos = "'.$repositorioArquivosTrocado.'";',
		44 =>'$numeroRegistrosPaginaRelatorio = "'.$numeroRegistrosPaginaRelatorioTrocado.'";',
		45 =>'$autoCadastro = "'.$autoCadastroTrocado.'";',
		46 =>'$SenhaGravada = "'.$SenhaGravada.'";',
		47 =>'$usuarioGravado = "'.$usuarioGravado.'";',
		48 =>'$metaDescricao = "'.$metaDescricaoTrocado.'";',
		49 =>'$metaPalavrasChave = "'.$metaPalavrasChaveTrocado.'";',
		50 =>'$metaAutor = "'.$metaAutorTrocado.'";',
		51 =>'$tagTituloSite = "'.$tagTituloSiteTrocado.'";',
		52 =>'$producao = "'.$producao.'";',
		53 =>'$linkRedirecionaRetorno = "'.$linkRedirecionaRetornoTrocado.'";',
		54 =>'$nomeEvento = "'.$nomeEventoTrocado.'";',
		55 =>'$preRequisitos = "'.$preRequisitosTrocado.'";'
		);
